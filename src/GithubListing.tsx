...
function RepositoryEntry({ repository } : {repository: Repository }) {
    return (
        <>
            ...
            <span className="text-gray-900 text-right hidden md:block">{repository.stargazers_count}</span>
            <span className="text-gray-900 ml-1 hidden md:block">👀</span>
            <span className="text-gray-900 text-right ml-6 hidden md:block">{repository.watchers}</span>
            <span className="text-gray-900 ml-1 hidden md:block">⭐</span>
        </>
    )
}
...
export function GithubListing() {
    ...
    const { error, data: repositories, isFetching } = useQuery<RepositoryResult>({
    ...
    return <div className="py-6 px-6 bg-white rounded-md gap-3 flex flex-col">
        ...
        {!isFetching && !error && repositories && repositories?.total_count > 0 && (
            <>
